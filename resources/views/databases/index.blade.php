        ...
        <div class="card">
            <div class="card-header d-flex align-items-center">
                ...
                <div class="ms-auto d-flex gap-2">
                    <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#createDatabaseModal"><i class="bx bx-plus me-1"></i> Agregar base de datos</button>
                    <a href="{{ route('export', 'databases') }}" class="btn btn-primary">Exportar Excel</a>
                </div>
            </div>
            <div class="card-body">
                <div class="mb-4">
                    <input type="text" id="search-database" class="form-control form-control-sm w-50" placeholder="Buscar por nombre, tipo o servidor">
                </div>
                <div class="table-responsive text-nowrap">
                    <table class="table align-middle">
                        ...
                        <tbody id="databases-search">
                            @include('databases.search', ['databases' => $databases, 'servers' => $servers, 'instances' => $instances, 'owners' => $owners])
                        </tbody>
                    </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch(url, {
                ...
                .then(res => {
                    if (!res.ok) throw new Error('Error al cargar la paginacion');
                    return res.json();
                })
                .then(data => {
                    if (typeof data.table !== 'string' || typeof data.pagination !== 'string') {
                        throw new Error(data.message || 'Respuesta invalida del servidor');
                    }
