                                        ...
                                        <td class="text-end">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#showServerModal{{ $server->id }}"><i class="bx bx-show me-1"></i> Ver</button>
                                                    <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#editServerModal{{ $server->id }}"><i class="bx bx-edit-alt me-1"></i> Editar</button>
                                                    <button class="dropdown-item text-warning" data-bs-toggle="modal" data-bs-target="#powerOffServerModal{{ $server->id }}"><i class="bx bx-power-off me-1"></i> Apagar</button>
                                                    <button class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#deleteServerModal{{ $server->id }}"><i class="bx bx-trash me-1"></i> Eliminar</button>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @if (session('success'))
                ...
                const rows = Array.isArray(importSummary) && importSummary.length
                    ? importSummary.map(item => `<li class="border-bottom py-2"><div class="d-flex justify-content-between"><span>${escapeHtml(item.label ?? 'Tabla')}</span><strong>${Number(item.total) || 0} registros</strong></div><div class="d-flex gap-3 small"><span class="text-success">Nuevos: ${Number(item.created) || 0}</span><span class="text-primary">Actualizados: ${Number(item.updated) || 0}</span></div></li>`).join('')
                    : '';
                Swal.fire({
                    icon: 'success',
                    title: 'Listo',
                    confirmButtonText: 'Perfecto',
                    ...(rows ? { html: `<div class="text-start mb-3"><p class="mb-2 fw-semibold">Resumen por tabla:</p><ul class="list-unstyled mb-0">${rows}</ul></div><p class="mb-0">${escapeHtml(successMessage)}</p>` } : { text: successMessage })
                });
            @endif
