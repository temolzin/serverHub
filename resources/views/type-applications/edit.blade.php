        ...
        <div class="modal-content">
            ...
            <form action="{{ route('type-applications.update', $type) }}" method="POST">
                ...
                <div class="modal-body text-start">
                    @php
                        $selectedType = strtoupper(old('type_application', $type->type_application));
                    @endphp
                    <div class="mb-4">
                        <label class="form-label fw-semibold">
                            <i class="bx bx-category me-1 text-primary"></i>Tipo de aplicación
                        </label>
                        <select name="type_application" class="form-select" required>
                            <option value="">Selecciona un tipo</option>
                            <option value="WEB" {{ $selectedType == 'WEB' ? 'selected' : '' }}>Web</option>
                            <option value="API" {{ $selectedType == 'API' ? 'selected' : '' }}>API</option>
                            <option value="WORKER" {{ $selectedType == 'WORKER' ? 'selected' : '' }}>Worker</option>
                            <option value="UNKNOWN" {{ $selectedType == 'UNKNOWN' ? 'selected' : '' }}>Desconocido</option>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            <input type="text" name="name_application" class="form-control" placeholder="Ej: Sistema de Nómina Corporativo" maxlength="100" value="{{ old('name_application', $type->name_application) }}" required>
                        </div>
